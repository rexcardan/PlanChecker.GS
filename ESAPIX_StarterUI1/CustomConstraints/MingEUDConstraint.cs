...
{
    public class MingEUDConstraint : IConstraint
    {
        private string _strId;
        private DoseValue _limit;
        private double _aValue;

        public MingEUDConstraint(string strId, DoseValue limit, double aValue)
        {
            _strId = strId;
            _limit = limit;
            _aValue = aValue;
        ...
        public ConstraintResult Constrain(PlanningItem pi)
        {
            ...
            // Check if gEUD is below the limit
            if (gEUD < _limit.Dose)
            {
                return new ConstraintResult(this, ResultType.ACTION_LEVEL_2, $"gEUD: {gEUD:N2} is below limit: {_limit}");
            }
            else
            {
                return new ConstraintResult(this, ResultType.PASSED, $"gEUD: {gEUD:N2} is not below limit: {_limit}");
            }
        }
    }
}